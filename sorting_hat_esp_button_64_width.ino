...
void checkButtons() {
    ...
    for (int i = 0; i < 4; i++) {
        switch (i) {
            case 0: buttonStates[i] = digitalRead(BUTTON_A); break;
            case 1: buttonStates[i] = digitalRead(BUTTON_B); break;
            case 2: buttonStates[i] = digitalRead(BUTTON_C); break;
            case 3: buttonStates[i] = digitalRead(BUTTON_D); break;
        }
    }
    for (int i = 0; i < 4; i++) {
        if (buttonStates[i] == LOW) {  // If the button is pressed (LOW because of INPUT_PULLUP)
            responses[questionIndex] = i + 1;  // Save user's answer (1–4)
            buttonPressed = true;

            int pin = (i == 0) ? BUTTON_A : (i == 1) ? BUTTON_B : (i == 2) ? BUTTON_C : BUTTON_D;
